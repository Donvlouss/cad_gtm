...
impl BSpline {
    ...
    pub fn interop(&self, u: f32) -> Vec3 {
        ...
        let knot_slice = self.knots.get_knots_bounds(knot_index, self.degree, self.is_periodic);
        let pole_index = Self::find_pole_index(&self.knots, knot_index, self.degree, self.is_periodic);
